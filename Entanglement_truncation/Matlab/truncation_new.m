...
e = zeros(1,chimax);
for chi = 1:chimax
    R = 1i*alpha/sqrt(2*Omega)*(diag(sqrt(1:chi-1),1)-diag(sqrt(1:chi-1),-1));
    Q = -(Omega+V/Omega)*diag(0:chi-1)/2 -  1/4*(Omega-V/Omega)*(diag(sqrt((2:chi-1).*(1:chi-2)),2)+diag(sqrt((2:chi-1).*(1:chi-2)),-2));
